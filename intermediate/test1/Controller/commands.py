...
def add_note():
    ...
    array_notes = lF.read_file() # type: ignore
    for i in array_notes: # type: ignore
        if Models.Note.Note.get_id(note) == Models.Note.Note.get_id(i):
            Models.Note.Note.set_id(note)
    array_notes.append(note) # type: ignore
    wF.write_file(array_notes, 'a') # type: ignore
...
def change_note():
    ...
    if flag:
        wF.write_file(array_notes_new, 'a') # type: ignore
